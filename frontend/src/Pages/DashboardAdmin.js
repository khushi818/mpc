...
const DashboardAdmin = () => {
  ...
  const drawer = (
    <div>
      ...
      <Divider />
      <List>
        {[{name : 'Role Admin' , icon : <PersonAddIcon /> , link : '/role'}, {name : 'VoteSession' , icon : <PollIcon/> , link : '/votesession'} ].map((fields, index) => (
          <Link to={fields.link} sx= {{ color : 'white' }}>
          <ListItem button key={fields.name} sx = {{ cursor : 'pointer'}}>
            <ListItemIcon>
              {fields.icon}
            </ListItemIcon>
